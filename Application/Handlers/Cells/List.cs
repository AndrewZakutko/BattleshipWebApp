...
namespace Application.Handlers.Cells
{
    public class List
    {
        ...
        public class Handler : IRequestHandler<Command, Result<List<Cell>>>
        {
            ...
            public async Task<Result<List<Cell>>> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                var listCellShipsDb = await _context.CellShips.Where(x => x.Field.Id == request.FieldId).ToListAsync();

                if(listCellShipsDb == null || listCellShipsDb.Count == 0)
                {
                ...
                var listCellsDb = listCellShipsDb.Select(x => x.Cell).ToList();
